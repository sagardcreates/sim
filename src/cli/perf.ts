...
const rows: string[] = ['| target agents | mean living (timed) | sim-years/s | ms per sim-day | ms per agent-day |', '|---|---|---|---|---|'];
for (const target of [200, 1000, 2000]) {
  const cfg = makeConfig({ init: { agentsPerClan: target / 4 } });
  ...
  rows.push(`| ${target} | ${meanLiving.toFixed(0)} | ${(years / secs).toFixed(2)} | ${((1000 * secs) / days).toFixed(2)} | ${((1e6 * secs) / (days * meanLiving)).toFixed(1)} µs |`);
