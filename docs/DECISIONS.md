...
- **2026-09-25 · Calling someone over brings them to you** (`comeUntil`): they walk straight toward the player for up to 4 sub-steps, stopping at water, then wait. The earlier version only made them stop where they were.
- **2026-09-25 · Walking together (companions, `escortUntil`).** Someone who likes you enough (odds from affinity, renown, loneliness and clanlessness) walks beside you for half a day. This lets you take a clan member away from their clan's eyes. Asking in public is itself seen ('walk' suspicion). Afterwards they walk home from wherever they are.
- **2026-09-25 · Courtship and marriage** use the sim's own pairing rules: opposite sex, both unpaired, adults, not kin, and within the age gap. Courting advances in proportion to the sim's `attraction`. A proposal's odds rise with courtship and affinity. On marriage `formPair` runs, and the spouse moves to your clan with their dependents; their kin then have kin in your clan (the kin term in clan valuation), which pulls them too. A married female player can bear children and never dies in childbirth. This is the route into established clans that helping alone could not open.
- **2026-09-25 · Renown** (0–1) is raised by gifts to the hungry, tending, big-game kills and marriage, and fades slowly. It adds to invite odds and to willingness to walk with you. It also brings visitors: before each sub-step there is a chance ∝ renown that someone in need (hungry, hurt), a clanless wanderer or a friend within 14 tiles comes to find the player.
- **2026-09-25 · Felled trees and kills are visible.** Wood cutting fells actual tree instances: they topple, lie as logs for 40 s, and leave stumps. The felling state (`felled`: tile → count, regrowing after 120 days) is sim state, so it replays. A kill leaves the animal lying where it fell for 45 s.
- **2026-09-25 · Island world for play** (`world.island`, `play.world`: 112² island). Elevation sinks toward the rim, so the map edge is open sea. Checked on 3 seeds: 8 years stable (185–207 people) and 4 camps placed. Research configs are unchanged.
- **2026-09-25 · Third-person game view.**
  - The camera is a perspective camera orbiting the player: drag to look, wheel for distance, all the way in to first person.
  - Ground-level rendering: a subdivided terrain with smooth shading, noise relief and blended colours (the height function matches the mesh); tall open-forest trees; sun shadows around the player; a sky dome with sunrise and sunset colour and night stars; fog; sea to the horizon; grass tufts near you; rocks; blob shadows under people; fog-aware custom shaders.
  - Trees between the camera and the player are hidden while they block the view.
  - The game page opens straight into play (remembering name, sex and world), with "New world…" in the help panel.
